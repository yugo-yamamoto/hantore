...
(function (root, factory) {
  ...
  var DEFS = [
    ...
    { id: 'body', label: '体の部位', icon: '🖐', hint: '頭から足まで・顔のパーツも',
      gloss: { '눈': '目', '배': 'おなか', '목': '首' },
      words: ['몸', '머리', '머리카락', '얼굴', '이마', '눈썹', '눈', '코', '입',
              '이빨', '혀', '귀', '목', '어깨', '팔', '손', '손가락', '손톱',
              '가슴', '배', '등', '허리', '무릎', '발', '발가락', '피', '뼈', '피부'] },

    { id: 'health', label: '体調・病気', icon: '🤒', hint: '病院で使う言葉',
      words: ['건강', '건강하다', '아프다', '병', '감기', '열', '기침',
              '다치다', '상처', '피곤하다',
              '병원', '의사', '간호사', '약', '약국',
              '주사', '치료', '수술', '낫다'] },

    { id: 'emotion', label: '感情・気持ち', icon: '😊', hint: '名詞・形容詞・動詞をまとめて',
      words: ['기분', '느낌', '기쁨', '슬픔', '걱정', '화', '웃음', '용기',
              '기쁘다', '슬프다', '즐겁다', '행복하다', '무섭다', '부끄럽다',
              '외롭다', '그립다', '반갑다',
              '울다', '웃다', '화내다', '놀라다', '사랑하다'] },

    { id: 'fruit-veg', label: '果物・野菜', icon: '🍎', hint: '市場で使う言葉',
      words: ['과일', '사과', '포도', '수박', '딸기', '바나나', '오렌지', '귤', '복숭아', '감', '참외',
              '채소', '배추', '무', '오이', '당근', '양파', '마늘', '파', '고추', '감자', '고구마'] },

    { id: 'animals', label: '動物', icon: '🐶', hint: '身近な動物から虫まで',
      gloss: { '말': '馬' },
      words: ['동물', '개', '고양이', '소', '돼지', '말', '닭', '오리', '새', '물고기',
              '토끼', '쥐', '호랑이', '곰', '사자', '코끼리', '원숭이', '뱀',
              '벌레', '모기', '파리', '나비', '거미'] },

    { id: 'korean-food', label: '韓国料理', icon: '🍚', hint: 'お店で頼むときの言葉',
      words: ['김치', '비빔밥', '불고기', '삼겹살', '김밥', '떡볶이', '냉면',
              '찌개', '국', '국수', '라면', '만두', '떡', '반찬', '김'] },

    { id: 'meal', label: '食事・主食', icon: '🍽', hint: '朝昼晩・ご飯とパン',
      words: ['음식', '밥', '빵', '쌀', '식사', '아침밥', '점심밥', '저녁밥',
              '간식', '메뉴', '맛'] },

    { id: 'meat-fish', label: '肉・魚・卵', icon: '🥩', hint: '市場とスーパーで使う',
      words: ['고기', '소고기', '돼지고기', '닭고기', '생선', '계란',
              '우유', '치즈', '버터', '콩'] },

    { id: 'drinks', label: '飲み物', icon: '🥤', hint: 'カフェと居酒屋で使う',
      gloss: { '차': 'お茶' },
      words: ['물', '커피', '차', '녹차', '주스', '콜라', '술', '맥주', '소주', '와인', '얼음'] },

    { id: 'sweets', label: 'お菓子・洋食', icon: '🍰', hint: '外来語が多いので読みやすい',
      words: ['과자', '사탕', '초콜릿', '아이스크림', '케이크',
              '피자', '치킨', '햄버거', '샌드위치'] },

    { id: 'seasoning', label: '調味料', icon: '🧂', hint: '台所で使う言葉',
      words: ['설탕', '소금', '간장', '고추장', '기름', '식초', '후추'] },
  ...
  /** テーマの問題数（12語以下なら2周、それ以上なら1周。10〜30問に収める） */
  function questionCount(id) {
    var p = pool(id);
    var n = p ? p.words.length : 0;
    var q = n <= 12 ? n * 2 : n;
    return Math.max(10, Math.min(30, q));
  }
